...
const Home = () => {
    ...
    const [peopleFiltred, setPeopleFiltred] = React.useState([]);
    ...
    const getPeoples = async () => {
        const response = await fetch(`${import.meta.env.VITE_API}/peoples?populate=*&sort=publishedAt:desc`, {
    ...
    React.useEffect(() => {
        if (search?.length) {
            const searchResults = people?.filter((item) => {
                const startsWith = item.attributes.name.toLowerCase().startsWith(search.toLowerCase()) ||
                ...
                const includes = item.attributes.name.toLowerCase().includes(search.toLowerCase()) ||
            ...
            if (currentTag !== '0') {
                const tagResults = people?.filter((item) => item.attributes.tag.data.id === parseInt(currentTag));
